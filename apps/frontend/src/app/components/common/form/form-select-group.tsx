...
type SelectOptionGroup<T extends OptionValue> = {
  label: string
...
export function FormSelectGroup<T extends OptionValue>(props: Props<T>) {
  const {
    allowClear,
    control,
    disabled,
          ...
          <div data-testid={`form-item-${inputName}`}>
            ...
            <div className={`relative form-select ${errors ? 'form-select-errors' : ''}`}>
              <Select<SelectOption<T>>
                ...
                ref={ref}
                value={sortedOptions?.flatMap((o) => o.options).filter((o) => o.value === value) || null}
              />
              {errors && (
